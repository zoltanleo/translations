...
- Запись arg1 Когда вы указываете переключатель -vu, компилятор сообщит вам, куда он записывает файл пакета. Неустранимая ошибка: не удается записать файл PCP Произошла ошибка при записи файла пакета. Неустранимая ошибка: ошибка чтения файла PCP. Это означает, что файл пакета поврежден и содержит недопустимую информацию. Потребуется перекомпиляция. Неустранимый: неожиданный конец файла PCP Неожиданный конец файла. Это может означать, что файл PCP поврежден. Неустранимая ошибка: недопустимая запись в PCP-файле: arg1 Модуль, который компилятор пытается прочитать, поврежден или сгенерирован с помощью более новой версии компилятора. Попытка использовать модуль, который был скомпилирован с другим режимом FPU. Попытка скомпилировать код при использовании модулей, которые не были скомпилированы в том же режиме формата с плавающей запятой. Либо весь код должен быть скомпилирован с включенной эмуляцией FPU, либо с выключенной эмуляцией FPU. Packagesearch: arg1 Когда вы используете параметр -vt, компилятор сообщает вам, где он пытается найти файлы пакета. Обязательный пакет arg1 Когда вы укажете ключ -vu, компилятор сообщит вам, какие пакеты требуются для этого пакета. Содержимое модуля arg1 Когда вы укажете ключ -vu, компилятор сообщит вам, какие модули содержит пакет. Ошибка: модуль arg1 уже содержится в пакете arg2 Модуль, указанный в секциях contains, не должен быть частью требуемого пакета. Обратите внимание, что модуль мог стать частью другого пакета, косвенно включив его. Предупреждение: модуль arg1 импортируется из косвенно необходимого пакета arg2. Если используется модуль из пакета, который не является частью раздела requires, тогда пакет должен требовать этот модуль напрямую, чтобы избежать путаницы. PPL filename arg1 Имя библиотеки двоичных пакетов, хранящейся в PCP.
