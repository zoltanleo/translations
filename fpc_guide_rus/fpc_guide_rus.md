...
*Автор перевода: Zoltanleo a.k.a Док* (последняя редакция 12.10.2020)
...
В этом разделе перечислены все сообщения, которые могут отображаться, если генератор кода обнаружил ошибку. 

- `Error: Parameter list size exceeds 65535 bytes` Процессор I386 ограничивает список параметров 65535 байтами. (Инструкция `RET` вызывает это.) 
- `Error: File types must be var parameters` Нельзя указывать файлы в качестве параметров-значений, т.е. они всегда должны быть объявлены как параметры `var`. 
- `Error: The use of a far pointer isn’t allowed there` Free Pascal не поддерживает дальние указатели, поэтому вы не можете получить адрес выражения, которое в результате имеет дальнюю ссылку. В результате конструкция `mem` имеет дальнюю ссылку, поэтому следующий код выдаст эту ошибку:
```pascal
var p : pointer;  
 ...  
 p:=@mem[a000:000];
```
- `Error: EXPORT declared functions cannot be called` Больше не используются. 
- `Warning: Possible illegal call of constructor or destructor` Компилятор обнаружил, что конструктор или деструктор вызывается внутри метода. Это, вероятно, приведет к проблемам, так как конструкторам/деструкторам требуются параметры на входе. 
- `Note: Inefficient code` Ваше выражение кажется компилятору сомнительным. 
- `Warning: unreachable code` Вы указали конструкцию, которая никогда не будет выполнена. Пример:
```pascal
while false do  
   begin  
   {.. code ...}  
...
```
- `Error: Abstract methods cannot be called directly` Вы не можете вызвать абстрактный метод напрямую. Вместо этого вы должны вызвать переопределяющий дочерний метод, потому что абстрактный метод не реализован. 
- `Register arg1 weight arg2 arg3` Отладочное сообщение. Показывается, когда компилятор рассматривает переменную для хранения в регистрах. 
- `Stack frame is omitted` Некоторым процедурам/функциям не нужен полный стековый фрейм, поэтому он опущен. Это сообщение будет отображаться при использовании ключа `-vd`. 
- `Error: Object or class methods cannot be inline` Вы не можете иметь встроенные методы объекта. 
- `Error: Procvar calls cannot be inline` Процедура с процедурным вызовом переменной не может быть встроенной. 
- `Error: No code for inline procedure stored` Компилятор не может сохранить код для встроенной процедуры. 
- `Error: Element zero of an ansi/wide- or longstring cannot be accessed, use (set)length instead` Вы должны использовать `setlength`, чтобы установить длину `ansi`/`wide`/`longstring` и `length`, чтобы получить длину такого типа строки. 
- `Error: Constructors or destructors cannot be called inside a 'with' clause` Внутри предложения `with` нельзя вызывать конструктор или деструктор для объекта, который есть в предложении `with`. 
- `Error: Cannot call message handler methods directly` Метод обработчика метода сообщения не может быть вызван напрямую, если он содержит явный аргумент `Self`. 
- `Error: Jump in or outside of an exception block` Запрещается входить или выходить из блока исключения, например `try..finally..end;`. Например, следующий код выдаст эту ошибку:
```pascal
label 1;  
 ...
 ...  
```
- `Error: Control flow statements are not allowed in a finally block` Недопустимо использовать операторы потока управления `break`, `continue` и `exit` внутри оператора `finally`. В следующем примере показана проблема:
```pascal
...  
   try  
      p;  
      ...
      ...  
      exit;  // Этот exit НЕ допускается  
   end;  
 ...  
```
Если процедура `p` вызывает исключение, выполняется блок `finally`. Если выполнение достигает выхода, неясно, что делать: выходить из процедуры или искать другой обработчик исключений.
- `Warning: Parameters size exceeds limit for certain cpu’s` Это указывает на то, что вы объявляете более 64 КБ параметров, которые могут не поддерживаться другими целевыми процессорами. 
- `Warning: Local variable size exceed limit for certain cpu’s` Это означает, что вы объявляете более 32 КБ локальных переменных, которые могут не поддерживаться другими целевыми процессорами. 
- `Error: Local variables size exceeds supported limit` Это означает, что вы объявляете более 32 КБ локальных переменных, что не поддерживается этим процессором. 
- `Error: BREAK not allowed` Вы пытаетесь использовать `break` вне конструкции цикла. 
- `Error: CONTINUE not allowed` Вы пытаетесь использовать `continue` вне конструкции цикла. 
- `Fatal: Unknown compilerproc "arg1". Check if you use the correct run time library` Компилятор ожидает, что библиотека времени выполнения содержит определенные подпрограммы. Если вы видите эту ошибку и не изменили код библиотеки времени выполнения, вполне вероятно, что используемая вами библиотека времени выполнения не соответствует используемому компилятору. Если вы изменили библиотеку времени выполнения, эта ошибка означает, что вы удалили подпрограмму, которая нужна компилятору для внутреннего использования. 
- `Fatal: Cannot find system type "arg1". Check if you use the correct run time library` Компилятор ожидает, что библиотека времени выполнения содержит определенные описания типов. Если вы видите эту ошибку и не изменили код библиотеки времени выполнения, вполне вероятно, что используемая вами библиотека времени выполнения не соответствует используемому компилятору. Если вы изменили библиотеку времени выполнения, эта ошибка означает, что вы удалили тип, который необходим компилятору для внутреннего использования. 
- `Hint: Inherited call to abstract method ignored` Это сообщение появляется только в режиме `Delphi`, когда вы вызываете абстрактный метод родительского класса с помощью `inherited;`. Вызов затем игнорируется. 
- `Error: Goto label "arg1" not defined or optimized away` Метка, используемая в описании `Goto`, не определяется или не оптимизируется удалением недоступного кода. 
- `Fatal: Cannot find type "arg1" in unit "arg2". Check if you use the correct run time library` Компилятор ожидает, что библиотека времени выполнения содержит определенные описания типов. Если вы видите эту ошибку и не изменили код библиотеки времени выполнения, вполне вероятно, что используемая вами библиотека времени выполнения не соответствует используемому компилятору. Если вы изменили библиотеку времени выполнения, эта ошибка означает, что вы удалили тип, который необходим компилятору для внутреннего использования. 
- `Error: Interprocedural gotos are allowed only to outer subroutines` Переходы между подпрограммами допускаются только в том случае, если переход переходит от внутренней к внешней подпрограмме или из подпрограммы в основную программу. 
- `Error: Label must be defined in the same scope as it is declared` В режиме `ISO` метки должны быть определены в той же области, в которой они объявлены. 
- `Error: Leaving procedures containing explicit or implicit exceptions frames using goto is not allowed` Нелокальные `goto` могут не использоваться для выхода из процедур, использующих исключения, как явные, так и неявные. Процедуры, которые используют автоматические типы, такие как `ansistrings` или конструкторы классов, также подвержены влиянию этого. 
- `Error: In ISO mode, the mod operator is defined only for positive quotient` В паскале `ISO` допускаются только положительные значения: `n mod m` действителен, только если `m>0`. 
- `Auto inlining: arg1` Из-за включенного автоматического встраивания компилятор автоматически вставляет эту подпрограмму. 
- `Error: The function used, is not supported by the selected instruction set: arg1` Некоторые функции не могут быть эффективно реализованы для определенных наборов инструкций, один пример - это умножение/сложение. Чтобы избежать очень неэффективного кода, компилятор жалуется в этом случае, поэтому либо выберите другой набор команд, либо замените вызов функции альтернативным кодом. 
- `Fatal: Maximum number of units (arg1) reached for the current target` В зависимости от целевой архитектуры, количество модулей ограничено. Этот предел был достигнут. Модуль считается только в том случае, если он содержит подсчет инициализаций или финализаций. 
- `Note: Call to subroutine "arg1" marked as inline is not inlined` Директива `inline` является лишь подсказкой компилятору. Иногда компилятор игнорирует эту подсказку; и подпрограмма, помеченная как встроенная, становится не встроенной. В этом случае эта подсказка дается. Компиляция с `-vd` может привести к появлению дополнительной информации, почему директива `inline` игнорируется.


### C.7 Ошибки этапа сборки/линковки.

В этом разделе перечислены ошибки, которые возникают, когда компилятор обрабатывает командную строку или обрабатывает файлы конфигурации. 
- `Warning: Source operating system redefined` Исходная операционная система переопределена. 
- `Info: Assembling (pipe) arg1` Сборка с использованием pipe(именованного канала) ко внешнему сборщику. 
- `Error: Can’t create assembler file: arg1` Указанный файл не может быть создан. Проверьте, есть ли у вас права доступа для создания этого файла. 
- `Error: Can’t create object file: arg1 (error code: arg2)` Указанный файл не может быть создан. Проверьте, есть ли у вас права доступа для создания этого файла. 
- `Error: Can’t create archive file: arg1` Указанный файл не может быть создан. Проверьте, есть ли у вас права доступа для создания этого файла. 
- `Error: Assembler arg1 not found, switching to external assembling` Программа на ассемблере не найдена. Компилятор создаст скрипт, который можно использовать для сборки и компоновки программы. 
- `Using assembler: arg1` Информационное сообщение о том, какой ассемблер используется.
- `Error: Error while assembling exitcode arg1` Произошла ошибка при сборке файла с использованием внешнего ассемблера. Обратитесь к документации инструмента ассемблера, чтобы узнать больше информации об этой ошибке. 
- `Error: Can’t call the assembler, error arg1 switching to external assembling` Произошла ошибка при вызове внешнего ассемблера. Компилятор создаст скрипт, который можно использовать для сборки и компоновки программы. 
- `Info: Assembling arg1` Информационное сообщение о том, какой файл собирается. 
- `Info: Assembling with smartlinking arg1` Информационное сообщение о том, какой файл собирается с использованием smartlinking. 
- `Warning: Object arg1 not found, Linking may fail !` Один из объектных файлов отсутствует, и связывание, вероятно, не удастся. Проверьте свои пути. 
- `Error: Error while linking` Общая ошибка при связывании. 
- `Error: Can’t call the linker, switching to external linking` Произошла ошибка при вызове внешнего компоновщика. Компилятор создаст скрипт, который можно использовать для сборки и компоновки программы. 
- `Info: Linking arg1` Информационное сообщение, показывающее, какая программа или библиотека прилинкована. 
- `Error: Util arg1 not found, switching to external linking` Внешний инструмент не найден. Компилятор создаст скрипт, который можно использовать для сборки и компоновки или постобработки программы. 
- `Using util arg1` Информационное сообщение, показывающее, какая внешняя программа (обычно постпроцессор) используется. 
- `Error: Creation of Executables not supported` Создание исполняемых программ не поддерживается для этой платформы, потому что это еще не было реализовано в компиляторе. 
- `Error: Creation of Dynamic/Shared Libraries not supported` Создание динамически загружаемых библиотек не поддерживается для этой платформы, поскольку оно еще не было реализовано в компиляторе. 
- `Error: Creation of Static Libraries not supported` Создание статических библиотек не поддерживается для этой платформы, так как это еще не было реализовано в компиляторе. 
- `Info: Closing script arg1` Информационное сообщение, отображаемое по завершении написания внешнего скрипта сборки и компоновки.
- `Error: resource compiler "arg1" not found, switching to external mode` Не найден компилятор внешних ресурсов. Компилятор создаст сценарий, который можно использовать для сборки, компиляции ресурсов и связывания или последующей обработки программы. 
- `Info: Compiling resource arg1` Информационное сообщение, показывающее, какой ресурс компилируется. 
- `unit arg1 cannot be statically linked, switching to smart linking` Было запрошено статическое связывание, однако был использован модуль, который не является статически связываемым. 
- `unit arg1 cannot be smart linked, switching to static linking` Было запрошено `smart`(интеллектуальное, умное) связывание , но использовался модуль, который не является `smart`(интеллектуально) связываемым. 
- `unit arg1 cannot be shared linked, switching to static linking` Была запрошено общее связывание, но был использован модуль, который не предназначен для общего связываения. 
- `Error: unit arg1 cannot be smart or static linked` Было запрошено интеллектуальное или статическое связывание, но использовался модуль, который нельзя использовать ни для одного из них. 
- `Error: unit arg1 cannot be shared or static linked` Было запрошено общее или статическое связывание, но использовался модуль, который нельзя использовать ни для одного из них. 
- `Calling resource compiler "arg1" with "arg2" as command line` Информационное сообщение, показывающее, какая командная строка используется для компилятора ресурсов. 
- `Error: Error while compiling resources` Компилятор или конвертер ресурсов вернул ошибку. 
- `Error: Can’t call the resource compiler "arg1", switching to external mode` Произошла ошибка при вызове компилятора ресурса. Компилятор создаст сценарий, который можно использовать для сборки, компиляции ресурсов и связывания или последующей обработки программы. 
- `Error: Can’t open resource file "arg1"` Произошла ошибка. Файл ресурсов не может быть открыт. 
- `Error: Can’t write resource file "arg1"` Произошла ошибка. Файл ресурсов не может быть записан. 
- `Note: File "arg1" not found for backquoted cat command` Компилятор не нашел файл, который должен быть расширен в параметры компоновщика. 
- `Warning: "arg1" not found, this will probably cause a linking failure` Компилятор добавляет определенные файлы кода запуска в компоновщик только тогда, когда они найдены. Если они не найдены, они не добавляются, и это может привести к ошибке линковки.
...
В этом разделе перечислены все сообщения, которые могут появляться, когда компилятор загружает модуль с диска в память. Многие из этих сообщений являются информационными сообщениями. 

- `Unitsearch: arg1` Когда вы используете опцию `-vt`, компилятор сообщает вам, где он пытается найти файлы модулей. 
- `PPU Loading arg1` Когда используется ключ `-vt`, компилятор сообщает, какие модули он загружает. 
- `PPU Name: arg1` Когда вы используете флаг `-vu`, отображается имя модуля. 
- `PPU Flags: arg1` Когда вы используете флаг `-vu`, отображаются флаги модулей. 
- `PPU Crc: arg1` Когда вы используете флаг `-vu`, показывается проверка CRC модуля. 
- `PPU Time: arg1` Когда вы используете флаг `-vu`, отображается время компиляции модуля. 
- `PPU File too short` Файл `ppufile` слишком короткий, присутствуют не все объявления.
- `PPU Invalid Header (no PPU at the begin)` Файл модуля содержит в качестве первых трех байтов коды ASCII символов PPU. 
- `PPU Invalid Version arg1` Этот файл модуля был скомпилирован с другой версией компилятора и не может быть прочитан. 
- `PPU is compiled for another processor` Этот файл модуля был скомпилирован для другого типа процессора и не может быть прочитан. 
- `PPU is compiled for another target` Этот файл модуля был скомпилирован для другой цели и не может быть прочитан. 
- `PPU Source: arg1` Когда вы используете флаг `-vu`, отображается имя исходного файла модуля. 
- `Writing arg1` Когда вы укажете ключ `-vu`, компилятор скажет вам, куда он записывает файл модуля. 
- `Fatal: Can’t Write PPU-File` Произошла ошибка при записи файла модуля. 
- `Fatal: Error reading PPU-File` Это означает, что файл модуля был поврежден и содержит неверную информацию. Будет необходима перекомпиляция. 
- `Fatal: unexpected end of PPU-File` Неожиданный конец файла. Это может означать, что файл PPU поврежден.
- `Fatal: Invalid PPU-File entry: arg1` Модуль, который пытается прочитать компилятор, поврежден или сгенерирован с более новой версией компилятора. 
- `Fatal: PPU Dbx count problem` В отладочной информации модуля есть несоответствие. 
- `Error: Illegal unit name: arg1 (expecting arg2)` Имя модул не соответствует имени файла. Причин может быть две: либо в написании есть ошибка в правописании, либо существует юнит с именем 8.3, где 8 символов равны первым 8 символам имени юнита с более длинным именем. Однако этот модуля не найден. **Пример**: программа содержит `uses mytestunit`; файл модуля или исходник `mytestunit` недоступны, но есть исходник с именем `mytestun`. Затем компилятор пытается скомпилировать и использовать его, однако ожидаемое имя модуля не совпадает. 
- `Fatal: Too much units` Free Pascal имеет ограничение до 1024 модулей в программе. Вы можете изменить это поведение, изменив константу `maxunits` в файле `fmodule.pas` компилятора и пересобрав компилятор. 
- `Fatal: Circular unit reference between arg1 and arg2` Два модуля используют друг друга в разделе `interface`. Это разрешено только в разделе `implementation`. По крайней мере, один модуль должен содержать другой в разделе `implementation`. 
- `Fatal: Can’t compile unit arg1, no sources available` Найден модуль, который нужно перекомпилировать, но нет доступных исходников. 
- `Fatal: Can’t find unit arg1 used by arg2` Вы пытались использовать модуль, чей файл PPU не найден компилятором. Проверьте свой конфигурационный файл для проверки соответствия путей модуля. 
- `Warning: Unit arg1 was not found but arg2 exists` Это сообщение об ошибке больше не используется. 
- `Fatal: Unit arg1 searched but arg2 found` Усечение в стиле DOS до 8 знаков для файлов PPU может привести к проблемам, если имя модуля длиннее 8 знаков. 
- `Warning: Compiling the system unit requires the -Us switch` При перекомпиляции модуля `system` (он требует специальной обработки) должен быть указан ключ `-Us`.
- `Fatal: There were arg1 errors compiling module, stopping` Когда компилятор встречает фатальную ошибку или слишком много ошибок в модуле, он останавливается с этим сообщением. 
- `Load from arg1 (arg2) unit arg3` Когда вы используете флаг `-vu`, отображается, какой модуль загружается из какого модуля. 
- `Recompiling arg1, checksum changed for arg2` Модуль перекомпилирован, потому что контрольная сумма модуля, от которого он зависит, изменилась. 
- `Recompiling arg1, source found only` При использовании флага `-vu`, эти сообщения говорят вам, почему текущий модуль был перекомпилирован. 
- `Recompiling unit, static lib is older than ppufile` Когда вы используете флаг `-vu`, компилятор предупреждает, если статическая библиотека модуля старше самого файла модуля. 
- `Recompiling unit, shared lib is older than ppufile` Когда вы используете флаг `-vu`, компилятор предупреждает, если общая библиотека модуля старше, чем сам файл модуля. 
- `Recompiling unit, obj and asm are older than ppufile` Когда вы используете флаг `-vu`, компилятор предупреждает, старше ли файл ассемблера или объектный файл самого файла модуля. 
- `Recompiling unit, obj is older than asm` Когда вы используете флаг `-vu`, компилятор предупреждает, если файл ассемблера модуля старше, чем объектный файл модуля. 
- `Parsing interface of arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он начинает анализировать раздел `interface` модуля. 
- `Parsing implementation of arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что начинает анализировать раздел `implementation` модуля. 
- `Second load for unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он начинает перекомпиляцию модуля во второй раз. Это может случиться с взаимозависимыми модулями.
- `PPU Check file arg1 time arg2` Когда вы используете флаг `-vu`, компилятор показывает имя файла, а также дату и время файла, от которого зависит перекомпиляция. 
- `Warning: Can’t recompile unit arg1, but found modified include files` Было обнаружено, что модуль изменил include-файлы, но поскольку некоторые исходные файлы не найдены, поэтому перекомпиляция невозможна. 
- `File arg1 is newer than the one used for creating PPU file arg2` Был найден измененный исходный файл для модуля компилятора. 
- `Trying to use a unit which was compiled with a different FPU mode` Попытка скомпилировать код при использовании модулей, которые не были скомпилированы с тем же режимом формата с плавающей точкой. Либо весь код должен быть скомпилирован с включенной эмуляцией `FPU`, либо с отключенной эмуляцией `FPU`. 
- `Loading interface units from arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он начинает загружать модули, определенные в разделе `interface` модуля. 
- `Loading implementation units from arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он начинает загружать модули, определенные в разделе `implementation` модуля. 
- `Interface CRC changed for unit arg1` При использовании флага `-vu` компилятор предупреждает, что вычисленный CRC для раздела `interface` был изменен после анализа раздела `implementation`. 
- `Implementation CRC changed for unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что вычисленный CRC был изменен после анализа раздела `implementation`. 
- `Finished compiling unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он завершил компиляцию модуля. 
- `Adding dependency: arg1 depends on arg2` Когда вы используете флаг `-vu`, компилятор предупреждает, что он добавил зависимость между двумя модулями.
- `No reload, is caller: arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он не будет перезагружать модуль, потому что - это модуль, который хочет загрузить этот же модуль (*т.е. сам себя, прим.перев.*). 
- `No reload, already in second compile: arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что не перезагрузит модуль, потому что он уже во второй перекомпиляции.
- `Flag for reload: arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он должен перезагрузить модуль. 
- `Forced reloading` Когда вы используете флаг `-vu`, компилятор предупреждает, что он перезагружает модуль, потому что это было необходимо. 
- `Previous state of arg1: arg2` Когда вы используете флаг `-vu`, компилятор показывает предыдущее состояние модуля. 
- `Already compiling arg1, setting second compile` Когда вы используете флаг `-vu`, компилятор предупреждает, что он начинает перекомпилировать модуль во второй раз. Это может случиться с взаимозависимыми модулями. 
- `Loading unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он начинает загрузку модуля. 
- `Finished loading unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он завершил загрузку модуля. 
- `Registering new unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он нашел новый модуль и регистрирует его во внутренних списках. 
- `Re-resolving unit arg1` Когда вы используете флаг `-vu`, компилятор предупреждает, что он должен пересчитать внутренние данные модуля. 
- `Skipping re-resolving unit arg1, still loading used units` Когда вы используете флаг `-vu`, компилятор предупреждает, что он пропускает пересчет внутренних данных модуля, потому что нет данных для пересчета. 
- `Unloading resource unit arg1 (not needed)` Когда вы используете флаг `-vu`, компилятор предупреждает, что он выгружает блок обработки ресурсов, поскольку ресурсы не используются.
- `Error: Unit arg1 was compiled using a different whole program optimization feedback input (arg2, arg3); recompile it without wpo or use the same wpo feedback input file for this compilation invocation` Когда модуль был скомпилирован с использованием whole program optimization (оптимизация всей программы, или `wpo`) (`-FW <x> -OW <x>`), эта скомпилированная версия модуля специализирована для этого конкретного сценария компиляции и не может использоваться в любом другом контексте. Его необходимо перекомпилировать прежде, чем вы сможете использовать его в другой программе или с другим входным файлом обратной связи `wpo`. 
...
В этом разделе перечислены ошибки, которые возникают, когда компилятор выполняет оптимизацию всей программы. 

- `Fatal: Cannot open whole program optimization feedback file "arg1"` Компилятор не может открыть указанный файл обратной связи со всей информацией об оптимизации программы. 
- `Processing whole program optimization information in wpo feedback file "arg1"` Компилятор начинает обработку всей информации об оптимизации программы, найденной в названном файле. 
- `Finished processing the whole program optimization information in wpo feedback file "arg1"` Компилятор завершил обработку всей информации об оптимизации программы, содержащейся в названном файле. 
- `Error: Expected section header, but got "arg2" at line arg1 of wpo feedback file` Компилятор ожидал заголовок раздела во всем файле оптимизации программы (начиная с `%`), но не нашел его. 
- `Warning: No handler registered for whole program optimization section "arg2" at line arg1 of wpo feedback file, ignoring` У компилятора нет обработчика для работы со всем упомянутым разделом информации об оптимизации программы, поэтому он проигнорирует его и перейдет к следующему разделу. 
- `Found whole program optimization section "arg1" with information about "arg2"` Компилятор обнаружил раздел со всей информацией по оптимизации программы, и, в соответствии с его обработчиком, этот раздел содержит информацию, используемую для упомянутой цели. 
- `Fatal: The selected whole program optimizations require a previously generated feedback file (use -Fw to specify)` Компилятору требуется информация, собранная во время предыдущего запуска компиляции, для выполнения выбранных оптимизаций всей программы. Вы можете указать расположение файла обратной связи, содержащего эту информацию, с помощью переключателя `-Fw`. 
- `Error: No collected information necessary to perform "arg1" whole program optimization found` Хотя вы указали компилятору файл, содержащий отчет об оптимизации всей программы, он не содержал информации, необходимой для выполнения выбранных оптимизаций. Скорее всего, вам придется перекомпилировать программу, используя соответствующий переключатель `-OWxxx`. 
- `Fatal: Specify a whole program optimization feedback file to store the generated info in (using -FW)` Вы должны указать файл обратной связи, в котором компилятор должен хранить всю обратную связь по оптимизации программы, которая создается во время выполнения компиляции. Это можно сделать с помощью переключателя `-FW`. 
- `Error: Not generating any whole program optimization information, yet a feedback file was specified (using -FW)` Компилятору было дано указание сохранять всю информацию об оптимизации программы в файле, указанном с помощью `-FW`, но не создавать на самом деле никаких отзывов об оптимизации всей программы. Классы генерируемой информации можно указать с помощью `-OWxxx`. 
- `Error: Not performing any whole program optimizations, yet an input feedback file was specified (using -Fw)` Компилятору не было дано указание выполнять какие-либо оптимизации всей программы (без параметров `-Owxxx`), но, тем не менее, был указан входной файл с такой обратной связью ( используя `-Fwyyy`). Поскольку это может указывать на то, что вы забыли указать параметр `-Owxxx`, компилятор в этом случае выдает ошибку. 
- `Skipping whole program optimization section "arg1", because not needed by the requested optimizations` Весь файл обратной связи по оптимизации программы содержит раздел с информацией, которая не требуется для выбранных оптимизаций всей программы. 
- `Warning: Overriding previously read information for "arg1" from feedback input file using information in section "arg2"` Файл обратной связи содержит несколько разделов, которые предоставляют один и тот же класс информации (например, информацию о том, какие виртуальные методы можно девиртуализировать). В этом случае используется информация из последнего встреченного раздела. Включите вывод отладки (`-vd`), чтобы увидеть, какой класс информации предоставляется в каждом разделе. 
- `Error: Cannot extract symbol liveness information from program when stripping symbols, use -Xs` Определенные сборщики жизнеспособности символов извлекают информацию о символах из связанной программы. Если информация символа удалена (опция `-Xs`), это невозможно. 
- `Error: Cannot extract symbol liveness information from program when when not linking` Некоторые коллекторы жизнеспособности символов извлекают информацию о символах из связанной программы. Если программа не связана компилятором, это невозможно. 
- `Fatal: Cannot find "arg1" or "arg2" to extract symbol liveness information from linked program` Некоторым сборщикам жизнеспособности символов требуется вспомогательная программа для извлечения информации о символах из связанной программы. Обычно это вспомогательная программа `nm`, которая является частью GNU binutils. 
- `Error: Error during reading symbol liveness information produced by "arg1"` Произошла ошибка во время чтения файла жизнеспособности символов, созданного с помощью программы `nm` или `objdump`. Причина может заключаться в том, что он был короче, чем ожидалось, или в том, что его формат не был понят. 
- `Fatal: Error executing "arg1" (exitcode: arg2) to extract symbol information from linked program` Некоторым сборщикам жизнеспособности символов требуется вспомогательная программа для извлечения информации о символах из связанной программы. Вспомогательная программа выдала сообщенный код ошибки при запуске в связанной программе. 
- `Error: Collection of symbol liveness information can only help when using smart linking, use -CX -XX` Является ли символ живым, определяется путем проверки того, существует ли он в окончательно связанной программе. Без интеллектуального связывания/удаления мертвого кода всегда включаются все символы, независимо от того, используются они на самом деле или нет. Таким образом, в этом случае все символы будут отображаться как живые, что делает эту оптимизацию неэффективной. 
- `Error: Cannot create specified whole program optimisation feedback file "arg1"` Компилятор не может создать файл, указанный с помощью параметра `-FW`, для хранения всей информации об оптимизации программы.



### C.13 Сообщения о загрузке пакетов.

В этом разделе перечислены все сообщения, которые могут возникнуть, когда компилятор загружает пакет с диска в память, сохраняет пакет из памяти на диск или при синтаксическом анализе пакетов в целом. Многие из этих сообщений являются информационными. 
- `Fatal: Can’t find package arg1` Вы пытались использовать пакет, файл PCP которого не найден компилятором. Найдите в файле конфигурации пути к пакетам. 
- `PCP file for package arg1 found` Найден файл PCP для указанного пакета. 
- `Error: Duplicate package arg1` Пакет уже был указан как обязательный и не может быть указан во второй раз. 
- `Error: Unit arg1 can not be part of a package` Модуль не может быть частью пакета, поскольку для модуля включена директива `DenyPackageUnit`. 
- `Note: Unit arg1 is implicitely imported into package arg2` Модуль не был указан как часть раздела `contains` и также не включен в один из требуемых пакетов. Добавьте модуль в раздел `contains`, чтобы повысить совместимость с другими пакетами. 
- `Fatal: Failed to create PCP file arg2 for package arg1` Не удалось создать файл PCP для пакета. 
- `Fatal: Failed to read PCP file for package arg1` Не удалось прочитать файл PCP для пакета. 
- `PCP loading arg1` Когда используется переключатель `-vt`, компилятор сообщает вам, какие пакеты он загружает. 
- `PCP Name: arg1` При использовании флага `-vu` отображается имя пакета. 
- `PCP Flags: arg1` При использовании флага `-vu` отображаются флаги пакета. 
- `PCP Crc: arg1` При использовании флага `-vu` отображается проверка CRC пакета. 
- `PCP Time: arg1` При использовании флага `-vu` отображается время компиляции пакета. 
- `PCP File too short` Файл PCP слишком короткий, присутствуют не все объявления. 
- `PCP Invalid Header (no PCP at the begin)` Файл пакета содержит в качестве первых трех байтов коды ASCII символов PCP. 
- `PCP Invalid Version arg1` Этот файл пакета был скомпилирован с другой версией компилятора и не может быть прочитан. 
- `PCP is compiled for another processor` Этот файл пакета был скомпилирован для другого типа процессора и не может быть прочитан. 
- `PCP is compiled for another target` Этот файл пакета был скомпилирован для другой цели и не может быть прочитан. 
- `Writing arg1` Когда вы указываете переключатель `-vu`, компилятор сообщает вам, куда он записывает файл пакета. 
- `Fatal: Can’t Write PCP-File` Произошла ошибка при записи файла пакета. 
- `Fatal: Error reading PCP-File` Это означает, что файл пакета поврежден и содержит недопустимую информацию. Потребуется перекомпиляция. 
- `Fatal: unexpected end of PCP-File` Неожиданный конец файла. Это может означать, что файл PCP поврежден. 
- `Fatal: Invalid PCP-File entry: arg1` Модуль, который компилятор пытается прочитать, поврежден или сгенерирован с помощью более новой версии компилятора. 
- `Trying to use a unit which was compiled with a different FPU mode` Попытка скомпилировать код при использовании модулей, которые не были скомпилированы в том же режиме формата с плавающей точкой. Либо весь код должен быть скомпилирован с включенной эмуляцией FPU, либо с выключенной эмуляцией FPU. 
- `Packagesearch: arg1` Когда вы используете параметр `-vt`, компилятор сообщает вам, где он пытается найти файлы пакета. 
- `Required package arg1` Когда вы укажете ключ `-vu`, компилятор сообщит вам, какие пакеты требуются для этого пакета. 
- `Contained unit arg1` Когда вы укажете ключ `-vu`, компилятор сообщит вам, какие модули содержит пакет. 
- `Error: Unit arg1 is already contained in package arg2` Модуль, указанный в секциях `contains`, не должен быть частью требуемого пакета. Обратите внимание, что модуль мог стать частью другого пакета посредством его косвенного включения. 
- `Warning: Unit arg1 is imported from indirectly required package arg2` `requires`, тогда пакет должен требовать этот модуль напрямую, чтобы избежать путаницы. 
- `PPL filename arg1` Имя библиотеки двоичных пакетов, хранящейся в PCP.



### C.14 Ошибки чтения ассемблера.

В этом разделе перечислены ошибки, генерируемые встроенным ридером ассемблера. Они не являются сообщениями самого ассемблера.

#### C.14.1 Общие ошибки ассемблера.

- `Divide by zero in asm evaluator` Эта фатальная ошибка возникает, когда константное выражение ассемблера выполняет деление на ноль. 
- `Evaluator stack overflow, Evaluator stack underflow` Об этой фатальной ошибке сообщается, когда константное выражение ассемблера слишком велико для анализа константным анализатором. Попробуйте уменьшить количество терминов. 
- `Invalid numeric format in asm evaluator` Эта фатальная ошибка сообщается, когда синтаксический анализатор констант обнаруживает нечисловое значение. Обычно эта ошибка никогда не возникает. 
- `Invalid Operator in asm evaluator` Об этой неустранимой ошибке сообщается, когда математический оператор обнаруживается синтаксическим анализатором констант. Обычно эта ошибка никогда не возникает. 
- `Unknown error in asm evaluator` Об этой неустранимой ошибке сообщается, когда синтаксический анализатор констант обнаруживает внутреннюю ошибку. Обычно эта ошибка никогда не возникает. 
- `Invalid numeric value` Это предупреждение выдается, когда преобразование из восьмеричного, двоичного или шестнадцатеричного числа в десятичное выходит за пределы поддерживаемого диапазона. 
- `Escape sequence ignored` Эта ошибка возникает, когда escape-последовательность, отличная от ANSI C, обнаруживается в строке C. 
- `Asm syntax error - Prefix not found` Это происходит при попытке использовать недопустимую инструкцию префикса. 
- `Asm syntax error - Trying to add more than one prefix` Это происходит, когда вы пытаетесь добавить более одной инструкции префикса. 
- `Asm syntax error - Opcode not found` Вы пытались использовать неподдерживаемый или неизвестный код операции. 
- `Constant value out of bounds` Эта ошибка появляется, когда синтаксический анализатор констант определяет, что используемое вами значение выходит за границы либо с использованием кода операции, либо с использованием объявления константы. 
- `Non-label pattern contains @` Это применимо только к ассемблеру в стиле `m68k` и `Intel`. Об этом сообщается, когда вы пытаетесь использовать идентификатор без метки с префиксом `@`.
...
- `Internal error in ConcatLabeledInstr()` Эти ошибки никогда не должны возникать. Если это так, значит, вы обнаружили новую ошибку в парсерах ассемблера. Обратитесь к одному из разработчиков. 
- `Opcode not in table, operands not checked` Это предупреждение появляется только при компиляции модуля ` system` или связанных файлов. Проверка операндов кодов операций не выполняется. 
- `@CODE and @DATA not supported` Эта конструкция Turbo Pascal не поддерживается. 
- `SEG and OFFSET not supported` Эта конструкция Turbo Pascal не поддерживается. 
- `Modulo not supported` Постоянная операция `Modulo` не поддерживается.
- `Floating point binary representation ignored`
- `Floating point hexadecimal representation ignored`
- `Floating point octal representation ignored` Эти предупреждения возникают, когда константа с плавающей точкой объявляется в базе, отличной от десятичной. Для этих форматов преобразование невозможно. Вместо этого вы должны использовать десятичное представление. 
- `Identifier supposed external` Это предупреждение появляется, если символ не найден в таблице символов. Поэтому он считается внешним. 
- `Functions with void return value can’t return any value in asm code` Только процедуры с возвращаемым значением могут иметь возвращаемое значение.
- `Error in binary constant`
- `Error in octal constant`
- `Error in hexadecimal constant`
- `Error in integer constant` Эти ошибки сообщаются, когда вы пытаетесь использовать недопустимое константное выражение или выражение, значение которого выходит за пределы допустимого диапазона.
...
- `Invalid operand type` Вы попытались использовать недопустимую комбинацию кода операции и операндов. Проверьте синтаксис и, если вы уверены, что он правильный, обратитесь к одному из разработчиков. 
- `Unknown identifier` Идентификатор, к которому вы пытаетесь получить доступ, не существует или не входит в текущую область.
- `Trying to define an index register more than once`
- `Trying to define a segment register twice`
- `Trying to define a base register twice` Вы пытаетесь определить регистр индекса/сегмента более одного раза. 
- `Invalid field specifier` Поле записи или объекта, к которому вы пытаетесь получить доступ, не существует или является неправильным.
- `Invalid scaling factor`
- `Invalid scaling value`
- `Scaling value only allowed with index` Допустимые значения масштабирования: 1,2,4 или 8. 
- `Cannot use SELF outside a method` Вы пытаетесь получить доступ к идентификатору `SELF` для объектов вне метода. 
- `Invalid combination of prefix and opcode` Эта инструкция не может использовать префикс для этого кода операции. 
- `Invalid combination of override and opcode` Этот код операции не может быть переопределен этой комбинацией. 
- `Too many operands on line` На `m68k` и `i386` существует не более трех инструкций операндов, вероятно, вы пытаетесь использовать недопустимый синтаксис для этого кода операции. 
- `Duplicate local symbol` Вы пытаетесь переопределить локальный символ, например локальную метку.
- `Unknown label identifer`
- `Undefined local symbol`
- `local symbol not found inside asm statement` Эта метка, похоже, не была определена в текущей области.
- `Assemble node syntax error`
- `Not a directive or local symbol` Оператор ассемблера недействителен, или вы не используете распознанную директиву.


#### C.14.2 Ошибки, специфические для I386

- `repeat prefix and a segment override on <= i386 ...` Может возникнуть проблема с прерываниями и инструкцией префикса, которая может привести к ложным результатам на компьютерах 386 и более ранних. 
- `Fwait can cause emulation problems with emu387` Это предупреждение появляется при использовании инструкции `FWAIT`. Это может вызвать проблемы с эмуляцией в системах, использующих эмулятор `em387.dxe`. 
- `You need GNU as version ¿= 2.81 to compile this MMX code` Код ассемблера MMX может быть скомпилирован только с использованием `GAS` v2.8.1 или новее.
- `NEAR ignored`
- `FAR ignored` `NEAR` и `FAR` игнорируются ассемблерами `Intel`, но все же принимаются для совместимости с 16-битной моделью кода.
- `Invalid size for MOVSX/MOVZX`
- `16-bit base in 32-bit segment`
- `16-bit index in 32-bit segment` 16-битная адресация не поддерживается. Вы должны использовать 32-битную адресацию. 
- `Constant reference not allowed` Запрещается пытаться адресовать постоянный адрес памяти в защищенном режиме. 
- `Segment overrides not supported` Переопределения сегментов в стиле `Intel` (например, `rep ds stosb`) не поддерживаются анализатором ассемблера. 
- `Expressions of the form [sreg:reg...] are currently not supported` Чтобы получить доступ к операнду памяти в другом сегменте, вы должны использовать синтаксис `sreg:[reg...]` вместо `[sreg:reg...]` 
- `Size suffix and destination register do not match` В синтаксисе `Intel AT&T` вы используете размер регистра, который не соответствует указанному размеру операнда.
...
- `Invalid constant expression/at beginning of line not allowed
NOR not supported`
...
#### C.14.3 Специфические ошибки m68k.

- `Increment and Decrement mode not allowed together` Вы пытаетесь использовать режим нарастания/убывания одновременно. 
- `Invalid Register list in movem/fmovem` Список регистров недействителен. Обычно диапазон регистров должен разделяться знаком `-`, а отдельные регистры должны разделяться косой чертой. 
...
- `1 Invalid function number` Была предпринята попытка вызвать недопустимую операционную систему. 
- `2 File not found` Сообщается при попытке удалить, переименовать или открыть несуществующий файл. 
- `3 Path not found` Сообщается процедурами обработки каталогов, когда путь не существует или недействителен. Также сообщается при попытке доступа к несуществующему файлу. 
- `4 Too many open files` Достигнуто максимальное количество файлов, открытых в данный момент вашим процессом. Некоторые операционные системы ограничивают количество файлов, которые могут быть открыты одновременно, и эта ошибка может возникать при достижении этого предела. 
- ` File access denied` В доступе к файлу отказано. Эта ошибка может быть вызвана одной из нескольких причин: 
    - попытка открыть для записи файл, который доступен только для чтения или который на самом деле является каталогом. 
    - Файл в настоящее время заблокирован или используется другим процессом. Попытка создать новый файл или каталог, когда файл или каталог с таким же именем уже существует. 
    - Попытка прочитать из файла, который был открыт в режиме только для записи. 
    - Попытка записи из файла, который был открыт в режиме только для чтения. 
    - Попытка удалить каталог или файл, пока это невозможно. 
    - Нет разрешения на доступ к файлу или каталогу. 
- `6 Invalid file handle` В этом случае используемая файловая переменная удаляется; это указывает на то, что ваша память повреждена. 
- `12 Invalid file access code` Сообщается, когда сброс или перезапись вызывается с недопустимым значением `FileMode`. 
- `15 Invalid drive number` Число, присвоенное функции `Getdir` или `ChDir`, указывает на несуществующий диск. 
- `16 Cannot remove current directory` Сообщение выдается при попытке удалить текущий активный каталог. 
- `17 Cannot rename across drives` Вы не можете переименовать файл так, чтобы он оказался на другом диске или разделе.
- `100 Disk read error` Произошла ошибка при чтении с диска. Обычно это происходит, когда вы пытаетесь прочитать после конца файла. 
- `101 Disk write error` Сообщение выдается, когда диск заполнен, и вы пытаетесь записать на него. 
- `102 File not assigned` Сообщение выдается процедурами `Reset`, `Rewrite`, `Append`, `Rename` и `Erase`, если вы вызываете их с неназначенным файлом в качестве параметра. 
- `103 File not open` Сообщение выдается процедурами и функциями: `Close`, `Read`, `Write`, `Seek`, `EOf`, `FilePos`, `FileSize`, `Flush`, `BlockRead` и `BlockWrite`, если файл не открыт. 
- `104 File not open for input` Сообщение выдается процедурами и функциями `Read`, `BlockRead`, `Eof`, `Eoln`, `SeekEof` или `SeekEoln`, если файл не открывается с помощью `Reset`. 
- `105 File not open for output` Сообщение выдается при записи, если текстовый файл не открывается с помощью `Rewrite`. 
- `106 Invalid numeric format` Сообщение выдается, когда из текстового файла считывается нечисловое значение, и ожидалось числовое значение. 
- `107 Invalid enumeration` Сообщение выдается, когда текстовое представление перечисляемой константы не может быть создано при вызове `str` или `write(ln)`.
- `150 Disk is write-protected` (Критическая ошибка)
- `151 Bad drive request struct length` (Критическая ошибка)
- `152 Drive not ready` (Критическая ошибка)
- `154 CRC error in data` (Критическая ошибка)
- `156 Disk seek error` (Критическая ошибка)
- `157 Unknown media type` (Критическая ошибка)
- `158 Sector Not Found` (Критическая ошибка)
- `159 Printer out of paper` (Критическая ошибка)
- `160 Device write fault` (Критическая ошибка)
- `161 Device read fault` (Критическая ошибка)
- `162 Hardware failure` (Критическая ошибка)
- `200 Division by zero` Программа попыталась разделить число на ноль. 
- `201 Range check error` Если вы скомпилировали свою программу с включенной проверкой диапазона, вы можете получить эту ошибку в следующих случаях: 
    - 1. Доступ к массиву осуществлялся с индексом за пределами объявленного диапазона. 
    - 2. Попытка присвоить значение переменной вне ее диапазона (например, перечислимого типа). 
- `202 Stack overflow error` Размер стека превышает максимальный (в этом случае размер локальных переменных следует уменьшить, чтобы избежать этой ошибки), или стек поврежден. Об этой ошибке сообщается только тогда, когда включена проверка стека. 
- `203 Heap overflow error` Куча вышла за пределы своих границ. Это происходит при попытке явно выделить память с помощью `New`, `GetMem` или `ReallocMem`, или когда создается экземпляр класса или объекта, а памяти не остается. Обратите внимание, что по умолчанию Free Pascal предоставляет растущую кучу, т.е. куча будет пытаться выделить больше памяти, если это необходимо. Однако, если размер кучи достиг максимального размера, разрешенного операционной системой или оборудованием, вы получите эту ошибку. 
- `204 Invalid pointer operation` Вы получите это в нескольких случаях: 
    - если вы вызываете `Dispose` или `Freemem` с недопустимым указателем, 
    - в случае, если вызывается `New` или `GetMem`, и больше нет доступной памяти. Поведение в этом случае зависит от настройки `ReturnNilIfGrowHeapFails`. Если это `True`, то возвращается `Nil`. Если `False`, возникает ошибка `runerror 204`.
- `205 Floating point overflow` Вы пытаетесь использовать или получить слишком большие действительные числа. 
- `206 Floating point underflow` Вы пытаетесь использовать или получить слишком маленькие действительные числа. 
- `207 Invalid floating point operation` Может произойти, если вы попытаетесь вычислить квадратный корень или логарифм отрицательного числа. 
- `210 Object not initialized` При компиляции с включенной проверкой диапазона программа сообщит об этой ошибке, если вы вызовете виртуальный метод, не вызвав конструктор его объекта. 
- `211 Call to abstract method` Ваша программа попыталась выполнить абстрактный виртуальный метод. Абстрактные методы должны быть переопределены, и должен быть вызван замещающий метод. 
- `212 Stream registration error` Это происходит, когда в модуле объектов зарегистрирован недопустимый тип. 
- `213 Collection index out of range` Вы пытаетесь получить доступ к элементу коллекции с недопустимым индексом (модуль `objects`). 
- `214 Collection overflow error` Коллекция достигла максимального размера, и вы пытаетесь добавить еще один элемент (модуль `objects`). 
- `215 Arithmetic overflow error` Об этой ошибке сообщается, когда результат арифметической операции выходит за пределы поддерживаемого диапазона. В отличие от Turbo Pascal, эта ошибка возникает только при 32-битном или 64-битном арифметическом переполнении. Это связано с тем, что перед выполнением фактической арифметической операции все конвертируется в 32-битную или 64-битную. 
- `216 General Protection fault` Приложение попыталось получить доступ к недопустимому пространству памяти. Это может быть вызвано несколькими проблемами: 
    - 1. Разыменование нулевого указателя. 
    - 2. Попытка получить доступ к памяти, которая находится вне пределов (например, вызов `move` с недопустимой длиной).
- `217 Unhandled exception occurred` Возникло исключение, но обработчик исключений отсутствует. Модуль `sysutils` задает обработчик исключений по умолчанию, который отлавливает все исключения и корректно завершает работу. 
- `218 Invalid value specified` Ошибка 218 возникает, когда для системного вызова было указано недопустимое значение, например, при указании отрицательного значения для вызова `seek()`. 
- `219 Invalid typecast` Вызывается при попытке неверного преобразования типа в классе с использованием оператора `as`. Эта ошибка также возникает, когда тип объекта или класса приводится к недопустимому классу или объекту и вызывается виртуальный метод этого класса или объекта. Эта последняя ошибка обнаруживается только в том случае, если используется параметр компилятора `-CR`. 
- `222 Variant dispatch error` Нет метода отправки для вызова из вариантного типа. 
- `223 Variant array create` Не удалось создать вариантный массив. Обычно когда не хватает памяти. 
- `224 Variant is not an array` Эта ошибка возникает, когда операция с вариантным массивом выполняется для вариантного типа, который не является массивом. 
- `225 Var Array Bounds check error` Эта ошибка возникает, когда индекс вариантного массива выходит за границы. 
- `227 Assertion failed error` Ошибка оператора контроля `Assert`, и процедурная переменная `AssertErrorProc` не была установлена. 
- `229 Safecall error check` Эта ошибка возникает из-за сбоя проверки `Safecall` и недоступности процедуры обработки. 
- `231 Exception stack corrupted` Эта ошибка возникает, когда объект исключения получен, но ни один из них не доступен. 
- `232 Threads not supported` Управление потоками зависит от отдельного драйвера в некоторых операционных системах (особенно в Unix). Модуль с этим драйвером должен быть указан в разделе `uses` программы, желательно как первый модуль (`cthread` в unix).
...
В таблице (F.1) перечислены доступные булевы директивы компилятора и соответствующие параметры командной строки. Другие директивы и соответствующие опции показаны в таблице (F.2). Дополнительную информацию о параметрах командной строки см. в [главе 5](##Конфигурация-компилятора.). Дополнительную информацию о директивах см.в [Руководстве программиста](https://www.freepascal.org/docs-html/current/prog/prog.html).



**Таблица F.1: Логические параметры и директивы**

| Короткое обозначение | Длинное обозначение     | Опция | Пояснение                                             |
| -------------------- | ----------------------- | ----- | ----------------------------------------------------- |
| $A[+/-]              | $ALIGN[ON/OFF]          |       | Выравнивание данных                                   |
| $B[+/-]              | $BOOLEVAL[ON/OFF]       |       | Логический режим оценки                               |
| $C[+/-]              | $ASSERTIONS[ON/OFF]     | -Sa   | Включить оператор контроля                            |
| $D[+/-]              | $DEBUGINFO[ON/OFF]      | -g    | Включить отладочную информацию                        |
| $E[+/-]              |                         |       | Эмуляция сопроцессора                                 |
| $F[+/-]              |                         |       | Функции far или near (игнорируется)                   |
| $G[+/-]              |                         |       | Сгенерировать код для процессора 80286 (игнорируется) |
|                      | $GOTO[ON/OFF]           | -Sg   | Поддержка операторов GOTO и Label                     |
|                      | $HINTS[ON/OFF]          | -vh   | Показывать подсказки                                  |
| $H[+/-]              | $LONGSTRINGS[ON/OFF]    | -Sh   | Использовать ansistring                               |
| $I[+/-]              | $IOCHECKS[ON/OFF]       | -Ci   | Проверка результатов операций ввода/вывода            |
|                      | $INLINE[ON/OFF]         | -Si   | Разрешить встроенный(inline) код                      |
| $L[+/-]              | $LOCALSYMBOLS[ON/OFF]   |       | Информация о локальных символах                       |
| $M[+/-]              | $TYPEINFO[ON/OFF]       |       | Генерирует RTTI для классов                           |
|                      | $MMX[ON/OFF]            |       | Поддержка Intel MMX                                   |
| $N[+/-]              |                         |       | Поддержка операций с плавающей точкой                 |
|                      | $NOTES[ON/OFF]          | -vn   | Разрешить пометки                                     |
| $O[+/-]              |                         |       | Поддержка оверлея (игнорируется)                      |
| $P[+/-]              | $OPENSTRINGS[ON/OFF]    |       | Поддержка открытых строк                              |
| $Q[+/-]              | $OVERFLOWCHECKS[ON/OFF] | -Co   | Проверка переполнения                                 |
| $R[+/-]              | $RANGECHECKS[ON/OFF]    | -Cr   | Проверки диапазона                                    |
| $S[+/-]              |                         | -Ct   | Провека стека                                         |
|                      | $SMARTLINK[ON/OFF]      | -CX   | Использовать умную линковку                           |
|                      | $STATIC[ON/OFF]         | -St   | Разрешить использование Static                        |
| $T[+/-]              | $TYPEDADDRESS[ON/OFF]   |       | Адресация типов                                       |
...
|                      | $ASMMODE            | -R    | Режим чтения ассемблера            |
|                      | $DEFINE             | -d    | Определение символа                |
|                      | $DESCRIPTION        |       | Задать описание программы          |
|                      | $ELSE               |       | Переключатель условной компиляции  |
|                      | $ENDIF              |       | Конец условной компиляции          |
|                      | $FATAL              |       | Сообщить о фатальной ошибке        |
|                      | $HINT               |       | Отправить сообщение-подсказку      |
| $I file              | $INCLUDE            |       | Включить файл или буквенный текст  |
|                      | $IF                 |       | Запуск условной компиляции         |
|                      | $IFDEF NAME         |       | Запуск условной компиляции         |
|                      | $IFNDEF             |       | Запуск условной компиляции         |
|                      | $IFOPT              |       | Запуск условной компиляции         |
|                      | $INCLUDEPATH        | -Fi   | Задать путь включения              |
|                      | $INFO               |       | Отправить информационное сообщение |
| $L file              | $LINK               |       | Связать объектный файл             |
|                      | $LIBRARYPATH        | -Fl   | Задать путь к библиотеке           |
|                      | $LINKLIB name       |       | Ссылка на библиотеку               |
| $M MIN,MAX           | $MEMORY             |       | Задать размер памяти               |
|                      | $MACRO              | -Sm   | Разрешить использование макросов   |
|                      | $MESSAGE            |       | Отправить сообщение                |
|                      | $MODE               |       | Задать режим совместимости         |
|                      | $NOTE               |       | Отправить заметку                  |
|                      | $OBJECTPATH         | -Fo   | Задать путь к объекту              |
|                      | $OUTPUT             | -A    | Задать выходной формат             |
|                      | $PACKENUM           |       | Размер перечислимого типа          |
|                      | $PACKRECORDS        |       | Выравнивание элемента записи       |
|                      | $SATURATION         |       | Насыщенность (игнорируется)        |
|                      | $STOP               |       | Остановить компиляцию              |
|                      | $UNDEF              | -u    | Неопределенный символ              |
...
# Приложение G.
## Получение последних исходников или установщиков.

Free Pascal находится в постоянном развитии. Время от времени создается новый набор установщиков с исходными кодами, которые считаются стабильными: это релизы. Их можно скачать с сайта Free Pascal. Загрузки обычно содержат исходники, из которых сделан релиз. 

Если по какой-то причине необходим более новый набор файлов - например, из-за того, что были исправлены определенные ошибки, мешающие правильной работе программы, можно загрузить последние исходные файлы и перекомпилировать их. 

Обратите внимание, что последние исходные коды могут или не могут компилироваться: иногда что-то ломается, и тогда загруженные исходники становятся бесполезными. Для веток (branche, или бранчей) с исправлениями (упомянутых ниже) исходные коды всегда должны компилироваться, поэтому лучше использовать только их. 

Получить последнюю версию можно тремя способами.


### G.1 Загрузка через Subversion.

Все исходники Free Pascal находятся в состоянии подрывной деятельности и могут быть загружены анонимно с сервера Subversion. С подходящим клиентом Subversion можно использовать следующие местоположения:
```bash
http://svn.freepascal.org/svn/fpc/trunk/
```
Этот репозиторий содержит последние исходные коды компилятора, RTL и пакетов. Это ветка активной разработки. 

Документация и все примеры из документации находятся в следующем репозитории
```bash
http://svn.freepascal.org/svn/fpcdocs/trunk/
```
Все файлы, необходимые для выпуска релиза, можно получить из
```bash
http://svn.freepascal.org/svn/fpcbuild/trunk/
```
Этот репозиторий содержит внешние ссылки на два других репозитория, а также все сценарии, демо и другие файлы, необходимые для создания новой версии Free Pascal. 

Free Pascal поддерживает ветку исправлений, которая используется для создания новых выпусков после изменения основной версии. Бранчи расположены в
```bash
http://svn.freepascal.org/svn/fpc/branches/fpc_X_Y
```
где X и Y составляют основной номер версии Free Pascal. Например, исправления, используемые для создания Free Pascal версий 2.6.x, доступны в
```bash
http://svn.freepascal.org/svn/fpc/branches/fixes_2_6
```
Архив Subversion зеркалируется на сервере
```bash
svn2.freepascal.org
```
...
Каждый день создается zip-архив, содержащий исходники в том виде, в каком они есть на сегодняшний день, они доступны на FTP-сайте:
```bash
...
```
а также ветки исправлений:
```bash
ftp://ftp.freepascal.org/pub/fpc/snapshot/fixes/source/fpc.zip
```
Создание zip-файлов - это автоматизированный процесс, поэтому эти файлы создаются каждый день.



### G.3 Загрузка снапшотов.

Некоторые члены команды Free Pascal также поддерживают устанавливаемые снапшоты(снимки состояния). Это установщики, сделанные из исходников того времени. Поскольку работа исходников не гарантируется, снапшот определенного дня может быть недоступен или лицо, ответственное за него, не имело возможности сделать его: эти снапшоты могут быть доступны, а могут и не быть. Их можно скачать с той же страницы, что и ZIP-архив с исходным кодом дня:
```bash
http://ftp.freepascal.org/develop.var
```
Снапшоты делаются как для ветки разработки, так и для ветки исправлений. Они доступны из
```bash
...
FTP-сайт является зеркальным, возможно, будет быстрее использовать зеркало.
