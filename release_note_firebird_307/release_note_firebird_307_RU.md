...
------
*Прим.переводчика*: на момент перевода в официальной документации допущена ошибка для WNET-протокола. "Правильная" строка коннекта должна выглядеть следующим образом:
...
INET преобразуется в TCP/IP, WNET - в именованные каналы Named Pipes, а XNET - в старый «локальный протокол Windows» (разделяемая память).

##### **Примеры**

**Подключение через TCP/IP с использованием имени базы данных**
```bash
...
**Подключение через TCP/IP с использованием нестандартного имени сервиса**
```bash
...
**Подключение через именованные каналы Named Pipes**
```bash
...
------
*Примечание переводчика*: на момент перевода в оригинальном документе примеры приведены с ошибкой. В данном документе приведен корректный синтаксис


**Loopback соединение через TCP/IP**
```bash
inet:///db/mydb.fdb
inet://C:\db\mydb.fdb
inet://mydb
...
**Loopback соединение через именованные каналы Named Pipes**
```bash
wnet://\\C:\db\mydb.fdb
wnet://\\mydb
```

**Локальное подключение с использованием общей памяти**
```bash
xnet://C:\db\mydb.fdb
xnet://mydb
```

**Локальное (встроенное по умолчанию) соединение**
```bash
/db/mydb.fdb
C:\db\mydb.fdb
mydb
...
Если host не указан, подразумевается локальное соединение. В зависимости от настроек, платформы и версии Firebird, это может быть выполнено через встроенный движок, протокол XNET (разделяемая память) или TCP/IP по локальной петле хоста.

На стороне сервера [конфигурация провайдера](###Провайдеры) по умолчанию имеет порядок `Remote`, `Engine12`, `Loopback`. Если *Remote* провайдеру не удается сопоставить строку подключения из-за отсутствия протокола или частей хоста, тогда *Engine12* (встроенный механизм) обрабатывает ее, как соединение без хоста. Чтобы подключиться локально с использованием определенного транспортного протокола, необходимо указать этот протокол:
```bash
inet://<путь к файлу базы данных или псевдоним>
```
или
```bash
wnet://\\<путь к файлу базы данных или псевдоним>
```
или
```bash
xnet://<путь к файлу базы данных или псевдоним>
```
​	<img src="D:\Archive\github\translations\release_note_firebird_307\pictures\Picto-info.png" style="zoom:25%;" />	|Протоколы WNET (именованные каналы Named Pipes) и XNET (общая память) доступны только для Windows.
